...
class RallyPieChartSegments {
  ...
  static List<RallyPieChartSegment> fromBudgetItems(List<BudgetItem> items) {
    var generator = (i) => RallyPieChartSegment(
        color: RallyColors.getBudgetColor(i),
        value: items[i].primaryAmount - items[i].amountUsed);
    return List<RallyPieChartSegment>.generate(items.length, generator);
  }

  static List<RallyPieChartSegment> _fromPrimaryAmounts(
      List<FinancialEntity> items, Color Function(int i) colorizer) {
    var generator = (i) => RallyPieChartSegment(
        color: colorizer(i), value: items[i].primaryAmount);
    return List<RallyPieChartSegment>.generate(items.length, generator);
  }
}
...
class _RallyPieChartState extends State<RallyPieChart>
  ...
  initState() {
    ...
    controller = AnimationController(
        duration:
            const Duration(milliseconds: Constants.defaultAnimationMillis * 3),
        vsync: this);
      ...
      child: SizedBox(
          ...
          child: Center(
              child: Column(
                  ...
                  children: <Widget>[
                Text(centerLabel),
                Text(Formatter.usdWithSign.format(centerAmount),
...
class _RallyPieChartOutlineBoxPainter extends BoxPainter {
  ...
  void paint(Canvas canvas, Offset offset, ImageConfiguration configuration) {
    ...
    double wholeMinusSpacesRadians =

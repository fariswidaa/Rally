...
class RallyLineChart extends StatelessWidget {
  const RallyLineChart({this.events});
...
class RallyLineChartPainter extends CustomPainter {
  ...
  // This is hardcoded to reflect the dummy data, but would be dynamic in a real app.
  // For demo only.
  final DateTime startDate = DateTime.utc(2020, 6, 20);
  ...
  void paint(Canvas canvas, Size size) {
    ...
    _drawXAxisTicks(
        canvas, Rect.fromLTWH(0.0, ticksTop, size.width, labelsTop - ticksTop));
  ...
  _drawLine(Canvas canvas, Rect rect) {
    ...
    for (int i = 0; i < numDays + smooth; i++) {
      ...
      List<DetailedEventItem> filteredEvents = events
          .where((e) =>
              startMillis <= e.date.millisecondsSinceEpoch &&
...
//            path.lineTo(x1, y1);
  ...
  _drawXAxisTicks(Canvas canvas, Rect rect) {
    ...
    for (int i = 0; i < numDays; i++) {
      ...
      canvas.drawRect(
          new Rect.fromPoints(Offset(x, i % 7 == tickShift ? rect.top : dayTop),
  ...
  _drawXAxisLabels(Canvas canvas, Rect rect) {
    TextPainter leftLabel = TextPainter(
      text: TextSpan(
          ...
          style: Theme.of(context).textTheme.bodyText2.copyWith(
              fontWeight: FontWeight.w700, color: RallyColors.gray25a)),
    ...
    centerLabel.layout();
    centerLabel.paint(
        canvas, Offset((rect.width - centerLabel.width) / 2, rect.center.dy));
